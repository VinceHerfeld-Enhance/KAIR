...
def _get_effective_gt_range(test_list_dir, folder_name, img_paths_gt):
    """Return (start, end) slice indices into img_paths_gt covering the LR frame range.

    Reads the LR frame list from ``test_list_dir/{folder_name}_im_list.txt``,
    extracts the frame numbers, and finds the first/last matching GT indices.
    """
    list_path = osp.join(test_list_dir, f"{folder_name}_im_list.txt")
    if not osp.isfile(list_path):
        return 0, len(img_paths_gt)

    with open(list_path, "r") as f:
        lr_frame_numbers = set()
        for line in f:
            line = line.strip()
            if line:
                n = _extract_frame_number(line)
                if n is not None:
                    lr_frame_numbers.add(n)

    if not lr_frame_numbers:
        return 0, len(img_paths_gt)

    gt_frame_numbers = [_extract_frame_number(osp.basename(p)) for p in img_paths_gt]
    matching_indices = [i for i, num in enumerate(gt_frame_numbers) if num in lr_frame_numbers]

    if not matching_indices:
        return 0, len(img_paths_gt)

    return matching_indices[0], matching_indices[-1] + 1


def _get_lr_frame_indices(test_list_dir, folder_name, img_paths_lq):
    """Return indices into img_paths_lq for frames listed in the LR frame list."""
    list_path = osp.join(test_list_dir, f"{folder_name}_im_list.txt")
    if not osp.isfile(list_path):
        return list(range(len(img_paths_lq)))
    ...
    lq_frame_numbers = [_extract_frame_number(osp.basename(p)) for p in img_paths_lq]
    return sorted([i for i, num in enumerate(lq_frame_numbers) if num in lr_frame_numbers])
...
class VideoRecurrentTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.test_list_dir = opt.get("test_list_dir", None)
        ...
        for subfolder_lq, subfolder_gt in zip(subfolders_lq, subfolders_gt):
            ...
            # Trim LQ and GT to only the frames that will be used
            if self.test_list_dir is not None:
                gt_start, gt_end = _get_effective_gt_range(self.test_list_dir, subfolder_name, img_paths_gt)
                img_paths_gt = img_paths_gt[gt_start:gt_end]
                lr_indices = _get_lr_frame_indices(self.test_list_dir, subfolder_name, img_paths_lq)
                img_paths_lq = [img_paths_lq[i] for i in lr_indices]

            max_idx = len(img_paths_lq)
            if self.test_list_dir is None:
            ...
            if self.cache_data:
                print(
                    ...
                    f"(LQ: {len(img_paths_lq)}, GT: {len(img_paths_gt)})"
                )
...
class VFI_DAVIS(data.Dataset):
    ...
    def __init__(self, data_root, ext="png"):
        ...
        self.transforms = transforms.Compose([transforms.CenterCrop((480, 840)), transforms.ToTensor()])
