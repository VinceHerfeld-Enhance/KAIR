...
def paired_random_crop(img_gts, img_lqs, gt_patch_size, scale, gt_path=None, min_variance=0.0, max_retries=5):
    ...
    Args:
        ...
        min_variance (float): Minimum variance threshold for LQ patches.
            If > 0, crops with variance below this are re-rolled up to
            max_retries times. Default: 0.0 (disabled).
        max_retries (int): Maximum number of re-rolls for low-variance
            crops. Default: 5.
    ...
    # optionally re-rolling low-variance crops
    n_tries = max_retries if min_variance > 0 else 0
    for _ in range(n_tries + 1):
        ...
        # compute variance on first LQ frame
        if input_type == "Tensor":
            patch = img_lqs[0][:, top : top + lq_patch_size, left : left + lq_patch_size]
            var = patch.var().item()
        else:
            patch = img_lqs[0][top : top + lq_patch_size, left : left + lq_patch_size, ...]
            var = patch.var()
