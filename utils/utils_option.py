...
def parse(opt_path, is_train=True):
    ...
    if "G_fft_loss_weight" not in opt["train"]:
        opt["train"]["G_fft_loss_weight"] = 0.0
    if "G_fft_loss_type" not in opt["train"]:
        opt["train"]["G_fft_loss_type"] = "l1"
    if "G_fft_mask_radius" not in opt["train"]:
        opt["train"]["G_fft_mask_radius"] = 0
    if "G_fft_mask_ratio" not in opt["train"]:
        opt["train"]["G_fft_mask_ratio"] = 0.1
